...
class bzr(scm):
    ...
    def detect_version(self, args, repodir):
        """Detection of version number for checked-out BZR repository."""
    ...
    def get_timestamp(self, args, repodir):
        log = self.helpers.safe_run(['bzr', 'log', '--limit=1',
                                     '--log-format=long'], repodir)[1]
        match = re.search(r'timestamp:(.*)', log, re.MULTILINE)
        if not match:
            return 0
        t = match.group(1).strip()
        timestamp = dateutil.parser.parse(t).strftime("%s")
